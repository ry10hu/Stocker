...
class files:
    ...
    with open('main/saves/1.json', "r") as file:
    ...
    def save(filename):
        ...
        if confirm == "y":    
            file = open(f"main/saves/{filename}.json", "w")
    ...
    def load(filename):
        ...
        if confirm == "y":
            files.save_r = json.load(open(f"main/saves/{filename}.json", "r"))

    def delete(filename):
        confirm = input(f"Delete progress in file {filename} (y/N)? ").lower()
        if confirm == "y":
            os.remove("main/saves/{filename}.json")